...
def get_session_id(df, interval):
    df_prev = df.shift()
    is_new_session = (df.userId != df_prev.userId) | (df.timestamp - df_prev.timestamp > interval)
...
def save_sessions(df, filepath):
    ...
    seqs = []
    labs = []
    for session_temp in sessions:
        session_temp = [int(i)+1 for i in session_temp.split(',')]
        for k in range(1, len(session_temp)):

            seqs.append(session_temp[:k])
            labs.append(session_temp[k])
    tra = (seqs, labs)

    pickle.dump(tra, open(filepath, 'wb'))
...
def save_dataset(dataset_dir, df_train, df_test):
    ...
    print(f'No. of Clicks: {len(df_train) + len(df_test)}')  ## Gowalla: 1122788; LastFM: 3835706
    print(f'No. of Items: {df_train.itemId.nunique()}')  ## Gowalla; LastFM: 38615
...
def preprocess_gowalla_lastfm(dataset_dir, csv_file, usecols, interval, n):
    dataset_dir = Path(dataset_dir.format(dataset=dataset_dir))
    print(f'reading {csv_file}...')
    ...
    df = df.dropna()
    df = update_id(df, 'userId')  ## userId recode
    df = update_id(df, 'itemId')  ## itemId recode
    df = df.sort_values(['userId', 'timestamp'])

    df = group_sessions(df, interval)  ## session id code
...
def main():
    preprocess_gowalla_lastfm('Gowalla', r'Gowalla/Gowalla_totalCheckins.txt', usecols=[0, 1, 4],
                              interval=Timedelta(days=1), n=30000)
    preprocess_gowalla_lastfm('LastFM', r'LastFM/lastfm-dataset-1K/userid-timestamp-artid-artname-traid-traname.tsv', usecols=[0, 1, 2],
                              interval=Timedelta(hours=8), n=40000)
